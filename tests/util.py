...
def create_octocat(
    ...
    return test_client.post(
        url_for("api.octocat_list"),
        headers={"Authorization": f"Bearer {access_token}"},
        data=f"name={octocat_name}&url={url}&age={age}",
        content_type="application/x-www-form-urlencoded",
    )


def retrieve_octocat_list(test_client, access_token, page=None, per_page=None):
    return test_client.get(
        url_for("api.octocat_list", page=page, per_page=per_page),
        headers={"Authorization": f"Bearer {access_token}"},
    )


def retrieve_octocat(test_client, access_token, octocat_name):
    return test_client.get(
        url_for("api.octocat", name=octocat_name),
        headers={"Authorization": f"Bearer {access_token}"},
    )


def update_octocat(test_client, access_token, octocat_name, url, age):
    return test_client.put(
        url_for("api.octocat", name=octocat_name),
        headers={"Authorization": f"Bearer {access_token}"},
        data=f"age={age}&url={url}",
        content_type="application/x-www-form-urlencoded",
    )


def delete_octocat(test_client, access_token, octocat_name):
    return test_client.delete(
        url_for("api.octocat", name=octocat_name),
        headers={"Authorization": f"Bearer {access_token}"},
    )
